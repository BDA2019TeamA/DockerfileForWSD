...
ADD ./files/juman-7.01.tar.bz2 ./
ADD ./files/jumanpp-1.01.tar.xz ./
ADD ./files/boost_1_71_0.tar.bz2 ./
ADD ./files/knp-4.19.tar.bz2 ./

RUN apt update
RUN apt install -y git aria2 curl wget bzip2 sudo make file xz-utils gcc g++ tar zlib1g-dev
...
make && make install
RUN source ~/.bash_profile && cd ~/cabocha-0.69/python && \
...
export LDFLAGS="-L$HOME/usr/lib" && \
export CPPFLAGS="-I$HOME/usr/include" && \
./bootstrap.sh && ./b2 install -j2 --prefix=$HOME/usr

# install juman++

RUN source ~/.bash_profile && cd ~/jumanpp-1.01 && \
export LDFLAGS="-L$HOME/usr/lib" && \
export CPPFLAGS="-I$HOME/usr/include" && \
./configure --prefix=$HOME/usr && make && make install
...
RUN source ~/.bash_profile && cd ~/knp-4.19 && \
export LDFLAGS="-L$HOME/usr/lib" && \
export CPPFLAGS="-I$HOME/usr/include" && \
./configure --prefix=$HOME/usr --with-juman-prefix=$HOME/usr && \
make && make install

# install pyknp

RUN source ~/.bash_profile && pip install pyknp
...
RUN apt install -y nano emacs vim
...
# install CrepeDB-access-tool
RUN source ~/.bash_profile && pip install git+https://github.com/BDA2019TeamA/CrepeDB
